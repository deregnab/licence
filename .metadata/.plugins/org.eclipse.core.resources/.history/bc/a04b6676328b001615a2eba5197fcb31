...
public class Mot {
  ...
  public int indiceMotifAutomate(Mot motif) {
	  ...
	  if (motif.length()==0){return 0;}
		  ...
		  for (int i=0;i<=this.length()-1;i++){
			  etat=autoMotif.transition(etat,this.toString().charAt(i));
  ...
  public int indiceMotifKMP(Mot motif){
	  if (motif.length()==0){
		  ...
		  int q=0;
		  for(int i=0;i<this.length()-1;i++){
			  while(q>0 && this.estFacteurDe(motif, i)!=motif.estFacteurDe(motif, q)){
				 q=pref[q];
			  }
			  if (motif.estFacteurDe(motif, q)==this.estFacteurDe(motif, i)){
				  q++;
			  }
			  if(q==motif.length()){
  ...
  private int[] calculerFonctionPrefixe(Mot motif){
    ...
    for(int q=2;q<=motif.length();q++){
    	while(k>0 && this.estFacteurDe(this, k) != this.estFacteurDe(this, q-1)){
    		k =pref[k];
    		
    	}
    	if (motif.estFacteurDe(this, k)==motif.estFacteurDe(this, q-1)){
    		k++;
